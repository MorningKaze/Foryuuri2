...
public final class Foryuuri extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        GlobalEventChannel.INSTANCE.subscribeAlways(GroupMessageEvent.class, (GroupMessageEvent event) ->{
            ...
            //vtbcheck
                ...
                if(isOp(id)){
                    String roomid = sentence.substring(sentence.lastIndexOf(" "));
                    if(SQL.delVtb(roomid, "" + groupId))
                        event.getSubject().sendMessage("已删除" + roomid);
                    else
                ...
                if(isOp(id)){
                    String roomid = sentence.substring(sentence.lastIndexOf(" "));
                    if(SQL.addVtb(roomid, "" + groupId))
                        event.getSubject().sendMessage("已添加" + roomid);
                    else
    ...
    */
    ...
    public static String checkVtb(String roomid) {
        try {
            String url = "https://live.bilibili.com/" + roomid;
            Document doc = Jsoup.connect(url).ignoreContentType(true).get();
            Element searchResult = doc.getElementsByClass("script-requirement").first();
            Element result2 = searchResult.selectFirst("script");
            String result3 = result2.html().replace("window.__NEPTUNE_IS_MY_WAIFU__=", "");
            JSONObject object = JSON.parseObject(result3);
            String result4 = object.getString("roomInfoRes");
            JSONObject object1 = JSONObject.parseObject(result4);
            String data = object1.getString("data");
            JSONObject dataObject = JSONObject.parseObject(data);
            String room_info = dataObject.getString("room_info");
            String anchor_info = dataObject.getString("anchor_info");

            JSONObject roomInfoObject = JSONObject.parseObject(room_info);
            JSONObject anchorInfoObject = JSONObject.parseObject(anchor_info);

            int live_status = roomInfoObject.getInteger("live_status");
            String title = roomInfoObject.getString("title");
            String name = JSONObject.parseObject(anchorInfoObject.getString("base_info")).getString("uname");

//            System.out.println("Live status: " + live_status);
//            System.out.println("Title: " + title);
//            System.out.println("Name: " + name);
            if (live_status == 1){
                if(!SQL.isLive(roomid)){
                    SQL.setLive(roomid, 1);
                    return name + "播了\n" + title + "\n" + url;
                }
                else
                    return "";
...
//            String liveStatus = responseString.substring(responseString.indexOf("\"liveStatus\":"), responseString.indexOf("\"liveStatus\":")+1);
...
//                    imageContent = imageContent.replaceAll("http://", "https://");
                imageContent = "https:" + imageContent.replaceAll("@100w_100h_1c.png", "");
